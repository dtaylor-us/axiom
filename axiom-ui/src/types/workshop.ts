...
export interface UtilityTreeNodeDto {
  nodeId: string;
  attributeName: string;
  refinement: string;
  scenarioId: string;
  scenarioTitle: string;
  businessImportance: 'H' | 'M' | 'L';
  technicalRisk: 'H' | 'M' | 'L';
  priorityLabel: string;
  rationale: string;
}

export interface UtilityTreeDto {
  generatedAtTurn: number;
  totalScenarios: number;
  architecturalDrivers: string[];
  nodes: UtilityTreeNodeDto[];
  generationRationale: string;
}

export interface ArchitectureImplicationDto {
  implicationId: string;
  sourceScenarioId: string;
  sourceScenarioTitle: string;
  implication: string;
  tradeoff: string;
  affectedQualityAttrs: string[];
  constraintType:
    | 'safety'
    | 'consistency'
    | 'availability'
