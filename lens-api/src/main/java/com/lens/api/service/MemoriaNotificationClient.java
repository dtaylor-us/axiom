...
public class MemoriaNotificationClient {
    ...
    public void notifyReviewComplete(UUID sessionId, ReviewReport report) {
        if (!enabled) {
            return;
        }
        try {
            webClient.post()
                    .uri("/api/v1/memoria/sessions/LENS/{sessionId}/distill", sessionId)
                    .header("X-Axiom-User-Id", "system")
                    .header("X-Axiom-Internal-Secret", internalSecret)
                    .bodyValue(Map.of(
                            "pillar", "LENS",
                            "sessionId", sessionId.toString(),
                            "sessionSummary", report.getExecutiveSummary() == null ? "" : report.getExecutiveSummary(),
                            "sessionPayload", objectMapper.convertValue(report, Map.class)))
                    .retrieve()
                    .bodyToMono(String.class)
                    .block(timeout);
        } catch (RuntimeException ex) {
            log.warn("Memoria distillation notification failed for Lens session {}", sessionId, ex);
        }
    }
    ...
    public Optional<Map<String, Object>> fetchSessionContext(UUID sessionId) {
        ...
        try {
            ...
            Map<String, Object> context = webClient.get()
                    .uri("/api/v1/memoria/sessions/LENS/{sessionId}/context", sessionId)
                    .header("X-Axiom-User-Id", "system")
                    .header("X-Axiom-Internal-Secret", internalSecret)
                    .retrieve()
            ...
            log.warn("Memoria context fetch failed for Lens session {}", sessionId, ex);
            return Optional.empty();
        }
    }
}
