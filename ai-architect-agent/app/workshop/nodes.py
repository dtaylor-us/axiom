    ...
    for raw_gap in parsed.get("new_gaps", []):
        ...
        try:
            gap = InformationGap(
                ...
                gap_question_type=raw_gap.get("gap_question_type", "unknown"),
                architectural_impact=raw_gap.get(
                    "architectural_impact", "informational"
                ),
                confidence_impact=float(raw_gap.get("confidence_impact", 0.0) or 0.0),
            )
...
async def elicit_scenarios_node(
    ...
    existing_ids = {s.scenario_id for s in state.deduplicated_scenarios}
    ...
    by_id: dict[str, WorkshopScenario] = {
        s.scenario_id: s for s in state.deduplicated_scenarios
    }
...
async def infer_attributes_from_scenarios_node(
    ...
    if not state.deduplicated_scenarios:
        return state
    ...
    prompt = load_prompt(
        ...
        scenarios=[s.model_dump(mode="json") for s in state.deduplicated_scenarios],
    ...
    scenario_implied_attrs = {
        attr.lower()
        for scenario in state.deduplicated_scenarios
        for attr in scenario.exercises_attributes
    }
    current_attr_names = {a.name.lower() for a in new_attributes}
    missing_from_scenarios = scenario_implied_attrs - current_attr_names

    if missing_from_scenarios:
        logger.warning(
            "COVERAGE GAP: Scenarios exercise attributes not in the "
            "attribute list. missing=%s session=%s",
            missing_from_scenarios,
            state.session_id,
        )

    for attr_name in sorted(missing_from_scenarios):
        from app.workshop.taxonomy import is_non_qa_concern, normalise_attribute_name

        if is_non_qa_concern(attr_name):
            continue
        canonical, _display_name = normalise_attribute_name(attr_name)
        supporting_scenarios = [
            scenario for scenario in state.deduplicated_scenarios
            if attr_name in [
                exercised.lower()
                for exercised in scenario.exercises_attributes
            ]
        ]
        new_attr = ElicitedAttribute(
            attribute_id=f"QA-{len(new_attributes) + 1:03d}",
            name=attr_name.title(),
            category=canonical,
            canonical_name=canonical,
            classification=(
                "canonical" if canonical == attr_name else "alias"
            ),
            description=(
                f"{attr_name.title()} implied by "
                f"{len(supporting_scenarios)} scenario(s)"
            ),
            importance="medium",
            confidence="tentative",
            evidence_quotes=[
                scenario.evidence_quote
                for scenario in supporting_scenarios
            ...
            scenarios=[
                ...
                for scenario in supporting_scenarios[:1]
            ],
            derived_in_turn=state.current_turn,
        )
        new_attributes.append(new_attr)
        logger.info(
            "Added scenario-implied attribute: %s confidence=tentative "
            "session=%s",
            attr_name,
            state.session_id,
        )
...
async def check_phase_transition_node(
    ...
    if state.workshop_phase == "scenario_brainstorm":
        grounded = sum(
            1 for s in state.deduplicated_scenarios
...
async def generate_utility_tree_node(
    state: WorkshopContext,
    config: RunnableConfig,
) -> WorkshopContext:
    """
    Node — generate or refresh the SEI QAW utility tree.

    Calls UtilityTreeGenerator.generate() which is guarded by
    WorkshopContext.has_sufficient_for_utility_tree. When there are
    fewer than 5 partial-or-better scenarios across 3 attributes,
    the existing tree is returned unchanged.

    Args:
        state:  Current WorkshopContext after consolidation.
        config: LangGraph config with 'utility_generator'.

    Returns:
        WorkshopContext with utility_tree populated or unchanged.
    """
    from app.workshop.utility_tree_generator import UtilityTreeGenerator

    generator: UtilityTreeGenerator | None = (
        config.get("configurable") or {}
    ).get("utility_generator")
    if generator is None:
        logger.warning(
            "utility_generator not in config — skipping utility tree. session=%s",
            state.session_id,
        )
        return state

    # Count eligible scenarios — same criterion as has_sufficient_for_utility_tree.
    eligible_count = sum(
        1 for s in state.deduplicated_scenarios
    ...
    # Skip the LLM call when an existing tree was built from the same evidence.
    # The tree only adds value when new scenarios have been elicited since the
    # last generation.  Regenerating on every turn wastes ~15 s of latency.
    if (
        state.utility_tree is not None
        and state.utility_tree.total_scenarios == eligible_count
    ):
        logger.debug(
            "Utility tree is current (eligible=%d). Skipping LLM. session=%s",
            eligible_count,
            state.session_id,
        )
        return state

    tree = await generator.generate(state)
    if tree is state.utility_tree:
        # Unchanged — either threshold not met or generation failed gracefully.
        return state
    return state.model_copy(update={"utility_tree": tree})


async def synthesise_implications_node(
    state: WorkshopContext,
    config: RunnableConfig,
) -> WorkshopContext:
    """
    Node — derive architectural implications from driver scenarios.

    Calls ImplicationSynthesiser.synthesise() which skips synthesis
    when no utility tree has been generated yet.

    Args:
        state:  WorkshopContext with utility_tree populated.
        config: LangGraph config with 'implication_synthesiser'.

    Returns:
        WorkshopContext with architecture_implications populated.
    """
    from app.workshop.implication_synthesiser import ImplicationSynthesiser

    synthesiser: ImplicationSynthesiser | None = (
        config.get("configurable") or {}
    ).get("implication_synthesiser")
    if synthesiser is None:
        logger.warning(
            "implication_synthesiser not in config — skipping implications. "
            "session=%s",
            state.session_id,
        )
        return state

    # Skip re-synthesis when the utility tree has not changed this turn.
    # Implications are derived from driver scenarios in the tree.  When the
    # tree is unchanged so are the drivers, so re-running the LLM adds no
    # value and costs ~15 s of latency.  Skip when:
    #   - the tree exists AND was generated in an earlier turn (not updated now)
    #   - AND we already have at least one implication from that tree
    if (
        state.utility_tree is not None
        and state.utility_tree.generated_at_turn < state.current_turn
        and len(state.architecture_implications) > 0
    ):
        logger.debug(
            "Implications current (tree from turn %d, now %d). Skipping LLM. session=%s",
            state.utility_tree.generated_at_turn,
            state.current_turn,
            state.session_id,
        )
        return state

    implications = await synthesiser.synthesise(state)
    return state.model_copy(update={"architecture_implications": implications})
