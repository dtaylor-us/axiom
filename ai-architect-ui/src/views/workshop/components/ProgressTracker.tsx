...
export function ProgressTracker({ currentPhase, turnNumber, hasSufficientAttributes }: Props) {
  ...
  return (
    <div
      className="flex items-center gap-2 px-4 py-2 bg-white border-b border-gray-200"
      data-testid="progress-tracker"
      aria-label="Workshop phase progress"
    >
      <span className="text-[11px] text-gray-500 font-medium mr-1">Phase</span>
        ...
        {PHASES.map((phase, idx) => {
          ...
          return (
            <div key={phase.key} className="flex items-center gap-1">
              ...
              {idx < PHASES.length - 1 && (
                <div className={`w-4 h-px ${done ? 'bg-emerald-300' : 'bg-gray-200'}`} aria-hidden="true" />
              )}
            </div>
          );
        })}
      ...
      <div className="flex items-center gap-2 ml-2">
        <span className="text-[11px] text-gray-500">Turn {turnNumber}</span>
        {hasSufficientAttributes && (
          <span className="rounded-full px-2.5 py-0.5 text-[11px] font-semibold bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200">
            Ready
          </span>
        )}
      </div>
