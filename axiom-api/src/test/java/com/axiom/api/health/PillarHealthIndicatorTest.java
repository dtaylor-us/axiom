...
class PillarHealthIndicatorTest {
    ...
    /**
     * Returns UP when Archon reports healthy status.
     */
    @Test
    @SuppressWarnings("unchecked")
    void returnsUpWhenArchonHealthEndpointReturns200() {
        archonServer.enqueue(new MockResponse().setResponseCode(200));

        PillarHealthIndicator indicator = new PillarHealthIndicator(WebClient.builder(), defaultRegistry(
                archonServer.url("/").toString(), true));
    ...
    void returnsOutOfServiceForDisabledPillars() {
        PillarHealthIndicator indicator = new PillarHealthIndicator(WebClient.builder(), defaultRegistry(
                archonServer.url("/").toString(), false));
    ...
    @Test
    @SuppressWarnings("unchecked")
    void returnsDegradedWhenArchonIsUnreachable() {
        PillarHealthIndicator indicator = new PillarHealthIndicator(WebClient.builder(), defaultRegistry(
                "http://127.0.0.1:6553", true));
    ...
    private PillarRegistryConfig defaultRegistry(String archonBaseUrl, boolean archonEnabled) {
        ...
        archon.setBaseUrl(archonBaseUrl);
        archon.setEnabled(archonEnabled);
