...
from app.workshop.nodes import (
    ...
    consolidation_node,
    generate_utility_tree_node,
    synthesise_implications_node,
...
class QualityAttributeWorkshopAgent:
    ...
    def __init__(self, llm_client: LLMClient) -> None:
        ...
        self._utility_generator = UtilityTreeGenerator(llm_client)
        self._implication_synthesiser = ImplicationSynthesiser(llm_client)
    ...
    def _build_graph(self) -> object:
        ...
        Node ordering is intentional and enforced by ADL-038 / scenario-first QAW:
          ...
          generate_utility_tree → synthesise_implications →
          check_transition → generate_response → END

        identify_gaps MUST precede scenario elicitation so the agent cannot
        treat speculation as evidence. The utility tree and implication nodes
        run after consolidation so they operate on the most complete evidence.
        """
        ...
        graph.add_node("generate_utility_tree", generate_utility_tree_node)
        graph.add_node("synthesise_implications", synthesise_implications_node)
        ...
        graph.add_edge("consolidation", "generate_utility_tree")
        graph.add_edge("generate_utility_tree", "synthesise_implications")
        graph.add_edge("synthesise_implications", "check_transition")
    ...
    async def process_turn(
        ...
        config = {
            "configurable": {
                ...
                "utility_generator":       self._utility_generator,
                "implication_synthesiser": self._implication_synthesiser,
            },
        }
    ...
    async def generate_from_current_evidence(
        ...
        prompt = load_prompt(
            ...
            scenarios=[
                s.model_dump() for s in context.deduplicated_scenarios
            ],
        )
    ...
    async def produce_summary(self, context: WorkshopContext) -> dict:
        ...
        """
        # Build a lean summary context that omits high-volume conversational
        # fields (raw_inputs, turns, resolved_answers) to stay within the
        # OpenAI 30,000 TPM limit. The prompt only needs structured artifacts
        # — not the full conversation history that produced them.
        summary_context = {
            "session_id":         context.session_id,
            ...
            "confirmed_attributes": context.confirmed_attributes,
            "scenarios": [
                s.model_dump() for s in context.deduplicated_scenarios
            ],
            "gaps": [
                g.model_dump() for g in context.gaps
                if not g.filled  # open gaps only — closed gaps add no value here
            ],
            "non_qa_concerns":    context.non_qa_concerns,
            "utility_tree":       (
                context.utility_tree.model_dump()
                if context.utility_tree else None
            ),
            "architecture_implications": [
                i.model_dump() for i in context.architecture_implications
            ],
